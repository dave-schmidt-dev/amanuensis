...
# --- Phase 2c M8: Hierarchize-cluster fixtures -------------------------
#
# Shared between cluster-enumeration, enqueue, and run_hierarchize_phase
# tests. Plants a workspace with an ultimate + two penultimates + atom
# children + linking edges + the bundled Walton snapshot, so the
# orchestrator helpers see realistic input.

_HIERARCHIZE_STABLE_AT = datetime(2026, 6, 1, 12, 0, 0, tzinfo=UTC)


def _hierarchize_role_attr() -> RoleAttribution:
    return RoleAttribution(
        agent=AgentAttribution(kind="llm", identifier="hierarchize", role="hierarchize"),
        activity="proposed",
        at=_HIERARCHIZE_STABLE_AT,
    )


def _build_hierarchize_atom(
    *,
    atom_id: str,
    source_id: str,
    narrative: str,
    ...
    return Atom(
        ...
        qualifier_basis=None,
        provenance_id="p-fixture00000010",
        role_attributions=[_hierarchize_role_attr()],
        schema_version=1,
    )


def _plant_hierarchize_atom(workspace: Path, atom: Atom) -> None:
    path = workspace / "distillations" / atom.source_id / "atoms" / f"{atom.id}.md"
    atomic_write_text(path, serialize_atom_md(atom))
...
def tmp_workspace_with_probandum_tree(tmp_path: Path) -> dict[str, str]:
    """Workspace with 1 ultimate + 2 penultimates + 2 atoms + linking edges.

    Each penultimate is linked upward to the ultimate via a
    probandum-edge AND has one atom-child via an outgoing
    probandum-edge. Walton-scheme snapshot is pinned. The returned dict
    maps role labels to real substrate ids so tests can reference them
    without re-deriving content-addressable hashes.
    """
    from amanuensis.schemas import Probandum, ProbandumEdge, compute_id

    marker = tmp_path / "amanuensis.yaml"
    marker.write_text(
        "schema_version: 1\nproject_name: t8-probandum-tree\n",
        encoding="utf-8",
    )
    ...
    role_attr = _hierarchize_role_attr()

    atom_pen1 = _build_hierarchize_atom(
        atom_id="a-pen1evidence001",
        source_id="src-A",
        narrative="Smith failed to deliver the April 2024 shipment.",
        char_offset=0,
    )
    atom_pen2 = _build_hierarchize_atom(
        atom_id="a-pen2evidence002",
        source_id="src-A",
        narrative="Smith refused to remit the late-payment fees.",
        char_offset=100,
    )
    _plant_hierarchize_atom(tmp_path, atom_pen1)
    _plant_hierarchize_atom(tmp_path, atom_pen2)
...
def tmp_workspace_with_orphan_penultimate(tmp_path: Path) -> Path:
    """Penultimate with NO upward link to an ultimate."""
    from amanuensis.schemas import Probandum, compute_id

    marker = tmp_path / "amanuensis.yaml"
    marker.write_text(
        "schema_version: 1\nproject_name: t8-orphan\n",
        encoding="utf-8",
    )
    (tmp_path / "distillations" / "src-A").mkdir(parents=True, exist_ok=True)
...
def tmp_workspace_with_childless_penultimate(tmp_path: Path) -> Path:
    """Penultimate with upward lineage but no outgoing children."""
    from amanuensis.schemas import Probandum, ProbandumEdge, compute_id

    marker = tmp_path / "amanuensis.yaml"
    marker.write_text(
        "schema_version: 1\nproject_name: t8-childless\n",
        encoding="utf-8",
    )
    (tmp_path / "distillations" / "src-A").mkdir(parents=True, exist_ok=True)
